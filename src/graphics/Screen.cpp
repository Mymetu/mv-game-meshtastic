...
int32_t Screen::runOnce()
{
    ...
    // Drive any active game one tick before redrawing the frame.
    if (gamesModule) {
        gamesModule->tick(millis());
    }
    ...
    if (targetFramerate != IDLE_FRAMERATE && ui->getUiState()->frameState == FIXED &&
        !(gamesModule && gamesModule->isActive())) {
        // oldFrameState = ui->getUiState()->frameState;
        // While a game is active we keep the higher framerate so the snake /
        // blink indicator stays visibly responsive. Otherwise fall back to the
        // battery-saving IDLE_FRAMERATE.
        targetFramerate = IDLE_FRAMERATE;
...
// Show detailed node views only on E-Ink builds
    ...
    if (!hiddenFrames.game) {
        fsi.positions.game = numframes;
        normalFrames[numframes++] = graphics::DebugRenderer::drawGameFrame;
    ...
    if (showingNormalScreen) {
        ...
        if (!inputIntercepted) {
            ...
            if (event->inputEvent == INPUT_BROKER_LEFT || event->inputEvent == INPUT_BROKER_ALT_PRESS) {
                if (gamesModule && gamesModule->handleInput(event->inputEvent)) {
                    // consumed by active game; skip frame navigation
                } else {
                    showFrame(FrameDirection::PREVIOUS);
                }
                ...
                // Active game locks the page; let it consume first.
                if (gamesModule && gamesModule->handleInput(event->inputEvent)) {
                    // consumed
                } else {
                ...
                if (gamesModule && gamesModule->handleInput(event->inputEvent)) {
                    // consumed by active game
                } else if (this->ui->getUiState()->currentFrame == framesetInfo.positions.home) {
                    cannedMessageModule->LaunchWithDestination(NODENUM_BROADCAST);
                }
            } else if (event->inputEvent == INPUT_BROKER_SELECT) {
                if (gamesModule && gamesModule->isActive() && gamesModule->handleInput(event->inputEvent)) {
                    // consumed by active game (e.g. restart on game over)
                } else if (this->ui->getUiState()->currentFrame == framesetInfo.positions.home) {
                ...
                setOn(false);
            } else if (event->inputEvent == INPUT_BROKER_CANCEL_LONG) {
                // Long press CANCEL: in-game = pause toggle; outside game = screen off
                if (gamesModule && gamesModule->isActive()) {
                    const bool wasPaused = gamesModule->isPaused();
                    gamesModule->togglePause();
                    // Only pop the pause banner when this press ENTERED Paused.
                    // If we were already paused, leave the existing banner alone.
                    if (!wasPaused && gamesModule->isPaused()) {
                        graphics::menuHandler::menuQueue = graphics::menuHandler::GamePauseBanner;
                        if (screen) screen->runNow();
                    }
                    // Force a fast refresh so the new state paints immediately,
                    // instead of waiting for the IDLE_FRAMERATE tick.
                    setFastFramerate();
                } else {
                    setOn(false);
                }
