...
void drawGameScreen(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    display->clear();
    display->setTextAlignment(TEXT_ALIGN_LEFT);
    display->setFont(FONT_SMALL);
    ...
    // === Header ===
    const char *titleStr = UI_STR("Game", "游戏");
    graphics::drawCommonHeader(display, x, y, titleStr);

    // === Body: numbered game list ===
    int line = 1;
    const int textX = x + 4;
    display->drawString(textX, getTextPositions(display)[line++], UI_STR("1. Snake",      "1. 贪吃蛇"));
    display->drawString(textX, getTextPositions(display)[line++], UI_STR("2. Flappy Bird","2. 像素小鸟"));
    display->drawString(textX, getTextPositions(display)[line++], UI_STR("3. ...",        "3. ..."));

    // === Footer hint ===
    display->setTextAlignment(TEXT_ALIGN_CENTER);
    display->drawString(display->getWidth() / 2 + x, display->getHeight() - FONT_HEIGHT_SMALL,
                        UI_STR("SELECT to play", "按 SELECT 开始"));
    display->setTextAlignment(TEXT_ALIGN_LEFT);
}

// ****************************
// *  Game Frame Dispatcher   *
// *  Idle → game list        *
// *  Active → running game   *
// ****************************
void drawGameFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x, int16_t y)
{
    if (gamesModule && gamesModule->isActive()) {
        gamesModule->draw(display, x, y);
    } else {
        drawGameScreen(display, state, x, y);
    }
}
