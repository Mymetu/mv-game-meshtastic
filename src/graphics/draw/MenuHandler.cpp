...
#ifdef ENABLE_GAMES_FRAME
    case GameFlappy:
        gameFlappyPlaceholder();
        break;
    case GameSnake:
        gameSnakePlaceholder();
        break;
#endif
...
#ifdef ENABLE_GAMES_FRAME
void menuHandler::gameMenu()
{
    enum optionsNumbers { Back, GameOptions, Snake, Flappy, enumEnd };
    static const char *optionsArray[enumEnd] = {UI_STR("Back", "返回")};
    static int optionsEnumArray[enumEnd] = {Back};
    int options = 1;

    optionsArray[options] = UI_STR("Options", "游戏选项");
    optionsEnumArray[options++] = GameOptions;

    optionsArray[options] = UI_STR("Snake", "贪吃蛇");
    optionsEnumArray[options++] = Snake;

    optionsArray[options] = UI_STR("Flappy Bird", "像素小鸟");
    optionsEnumArray[options++] = Flappy;

    BannerOverlayOptions bannerOptions;
    bannerOptions.message = UI_STR("Game", "游戏");
    if (currentResolution == ScreenResolution::UltraLow) {
        bannerOptions.message = UI_STR("Game", "游戏");
    }
    bannerOptions.optionsArrayPtr = optionsArray;
    ...
    bannerOptions.bannerCallback = [](int selected) -> void {
        if (selected == Flappy) {
            menuHandler::menuQueue = menuHandler::GameFlappy;
            screen->runNow();
        } else if (selected == Snake) {
            menuHandler::menuQueue = menuHandler::GameSnake;
            screen->runNow();
        } else if (selected == GameOptions) {
            menuHandler::menuQueue = menuHandler::GameOptionsMenu;
            screen->runNow();
        } else {
            // Back: 关闭 banner，回到 home frame
            menuQueue = MenuNone;
        }
    };
    screen->showOverlayBanner(bannerOptions);
...
void menuHandler::gameFlappyPlaceholder()
{
    if (gamesModule) {
        gamesModule->startFlappy();
        // Trigger a redraw; player should already be on or will land on the game frame.
        if (screen) screen->forceDisplay(true);
    } else {
        screen->showSimpleBanner(UI_STR("Flappy Bird\nNot available", "像素小鸟\n不可用"), 1500);
    }
    // Close the menu banner either way.
    menuQueue = MenuNone;
    if (screen) screen->runNow();
}

void menuHandler::gameSnakePlaceholder()
{
    if (gamesModule) {
        gamesModule->startSnake();
        // Trigger a redraw; player should already be on or will land on the game frame.
        if (screen) screen->forceDisplay(true);
    } else {
        screen->showSimpleBanner(UI_STR("Snake\nNot available", "贪吃蛇\n不可用"), 1500);
    }
    // Close the menu banner either way.
    menuQueue = MenuNone;
    if (screen) screen->runNow();
...
void menuHandler::gameOverBanner()
{
    enum optionsNumbers { Back, Continue, enumEnd };
    static const char *optionsArray[enumEnd] = {UI_STR("Quit", "退出")};
    static int optionsEnumArray[enumEnd] = {Back};
    int options = 1;

    optionsArray[options] = UI_STR("Continue", "继续");
    optionsEnumArray[options++] = Continue;

    BannerOverlayOptions bannerOptions;
    bannerOptions.message = UI_STR("Game Over", "游戏结束");
    if (currentResolution == ScreenResolution::UltraLow) {
        bannerOptions.message = UI_STR("Game Over", "已结束");
    }
    bannerOptions.optionsArrayPtr = optionsArray;
    bannerOptions.optionsCount = options;
    bannerOptions.optionsEnumPtr = optionsEnumArray;
    bannerOptions.bannerCallback = [](int selected) -> void {
        if (selected == Continue) {
            // Restart the snake on the same module (seeds, score reset).
            if (gamesModule) {
                gamesModule->restart();
            }
            menuQueue = MenuNone;
            ...
            // Quit: drop back to Idle so the game list shows again.
            if (gamesModule) {
                gamesModule->quit();
            }
            menuQueue = MenuNone;
...
void menuHandler::gamePauseBanner()
{
    enum optionsNumbers { Continue, Quit, enumEnd };
    static const char *optionsArray[enumEnd] = {UI_STR("Continue", "继续")};
    static int optionsEnumArray[enumEnd] = {Continue};
    int options = 1;

    optionsArray[options] = UI_STR("Quit", "退出");
    optionsEnumArray[options++] = Quit;

    BannerOverlayOptions bannerOptions;
    bannerOptions.message = UI_STR("Paused", "已暂停");
    if (currentResolution == ScreenResolution::UltraLow) {
        bannerOptions.message = UI_STR("Paused", "暂停");
    }
    bannerOptions.optionsArrayPtr = optionsArray;
    bannerOptions.optionsCount = options;
    bannerOptions.optionsEnumPtr = optionsEnumArray;
    // Default highlight "Continue" so SELECT resumes the game immediately.
    bannerOptions.InitialSelected = Continue;
    bannerOptions.bannerCallback = [](int selected) -> void {
        // Continue: resume playing WITHOUT resetting snake state (keeps score/progress).
        if (selected == Continue) {
            if (gamesModule) {
                gamesModule->resume();
            }
        } else { // Quit: only other option
            if (gamesModule) {
                gamesModule->quit();
            }
        }
...
{
    enum optionsNumbers { Back, Buzzer, enumEnd };
    static const char *optionsArray[enumEnd] = {UI_STR("Back", "返回")};
    static int optionsEnumArray[enumEnd] = {Back};
    int options = 1;

#if !defined(DISABLE_BUZZER)
    // 蜂鸣器开关：根据当前状态显示 label
    const bool buzzerOn = (config.device.buzzer_mode != meshtastic_Config_DeviceConfig_BuzzerMode_DISABLED);
    optionsArray[options] = buzzerOn ? UI_STR("Buzzer: On  (click to off)", "蜂鸣器:开 (点关闭)")
                                    : UI_STR("Buzzer: Off (click to on)",  "蜂鸣器:关 (点打开)");
    optionsEnumArray[options++] = Buzzer;
#endif

    BannerOverlayOptions bannerOptions;
    bannerOptions.message = UI_STR("Game Options", "游戏选项");
    if (currentResolution == ScreenResolution::UltraLow) {
        bannerOptions.message = UI_STR("Options", "选项");
    }
    bannerOptions.optionsArrayPtr = optionsArray;
    bannerOptions.optionsCount = options;
    bannerOptions.optionsEnumPtr = optionsEnumArray;
